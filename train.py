...
from loss import face_detector_loss_single_target
from valid import *
...
def main():
    input_shape = (96, 96, 3)
    json_dir = r"D:\Dataset\faceV3\Labelss"
    pts_num = 16
    initial_lr = 1e-3
    eta_min = 1e-8
    warmup_epochs = 0
    ...
    patience = 30
    # 初始化模型
    model = FaceDetector(input_shape=input_shape, pts_num=pts_num)

    # 损失函数
    loss_fn = face_detector_loss_single_target()
    ...
    callbacks = [
        ...
        KeypointErrorCallback(val_dataset, log_dir="./logs")
    ]
    ...
    def representative_data_gen():
        for images, _ in train_dataset.take(500):  # 取 500 batch 样本
            for img in images:
                img = tf.cast(img, tf.float32)  # [0,1] or [-1,1]
