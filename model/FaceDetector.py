...
def spatial_attention(inputs):
    # 生成 1 通道注意力图
    attention = layers.Conv2D(1, kernel_size=3 , strides=1, padding='same', activation='sigmoid')(inputs)
...
def FaceDetector(input_shape=(60, 80, 3), pts_num=16):
    inputs = keras.layers.Input(shape=input_shape)
    out_channels = pts_num * 3

    # 添加注意力到每一层
    x = depthwise_separable_conv(inputs, 3, 32, 2, use_spatial_att=False) # 48
    x = bottleneck_block(x, 32)
    x = depthwise_separable_conv(x, 3, 64, 2, use_spatial_att=False) # 24
    x = bottleneck_block(x, 64)
    x = depthwise_separable_conv(x, 3, 128, 2, use_spatial_att=False) # 12
    x = bottleneck_block(x, 128)
    x = depthwise_separable_conv(x, 3, 256, 2, use_spatial_att=False)  # 6
...
if __name__ == "__main__":
    model = FaceDetector(input_shape=(96, 96, 3))
    model.summary()
